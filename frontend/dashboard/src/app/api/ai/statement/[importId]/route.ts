...
export async function GET(
  ...
  try {
    ...
    const storageResolution = await resolveStatementStorageForImport({
      supabase,
      importId,
      householdId: profile.household_id,
    })
    const storageBucket = storageResolution?.storageBucket ?? readString(fileImport.storage_bucket)
    const storagePath = storageResolution?.storagePath ?? readString(fileImport.storage_path)
