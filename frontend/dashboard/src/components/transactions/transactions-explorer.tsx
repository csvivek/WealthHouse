...
type DisplayRow =
  | { type: 'chain'; chain: TransferChainGroup<StatementTxn>; date: string }
  | { type: 'txn'; txn: StatementTxn; date: string }
...
function TransactionListRow({
  ...
  role?: 'origin' | 'passthrough' | 'terminal'
  depth?: number
  showCheckbox?: boolean
}) {
  const merchantName = txn.merchant_normalized ?? txn.merchant_raw ?? txn.description ?? 'Unknown'
  const rolePresentation = role ? getRolePresentation(role) : null

  return (
    <div
      className={cn(
        'group rounded-[1.15rem] border border-border/65 bg-background/35 p-4 transition-colors hover:border-primary/20 hover:bg-accent/20',
        depth > 0 && 'border-border/45 bg-background/20',
      )}
        ...
        <div className="flex min-w-0 flex-1 gap-3">
          {showCheckbox ? (
            ...
            <div className="mt-0.5 flex w-5 justify-center">
              {rolePresentation ? (
                <span className={cn('inline-flex h-5 w-5 items-center justify-center rounded-full border text-[11px] font-semibold', rolePresentation.className)}>
                  {rolePresentation.glyph}
                </span>
              ) : null}
            </div>
          )}
          ...
          <div className="min-w-0 flex-1 space-y-2">
            <div className="flex flex-wrap items-center gap-2">
              <span className="font-mono text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{formatDateShort(txn.txn_date)}</span>
              {rolePresentation ? <Badge variant="outline">{rolePresentation.label}</Badge> : null}
              <AccountPill account={account} />
            </div>

            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <p className="truncate font-medium text-foreground">{merchantName}</p>
                <Pencil className="size-3.5 shrink-0 text-muted-foreground opacity-0 transition-opacity group-hover:opacity-100" />
              </div>
              {txn.description && txn.description !== merchantName ? (
                <p className="line-clamp-2 text-sm text-muted-foreground">{txn.description}</p>
              ) : null}
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              />
              {transferHint ? <span className="text-xs text-muted-foreground">{transferHint}</span> : null}
            </div>

            <div className="flex flex-wrap gap-1">
              {txn.tags.length > 0
                ? txn.tags.map((tag) => <TagBadge key={tag.id ?? tag.name} {...tag} className="text-[11px]" />)
                : <span className="text-xs text-muted-foreground">No tags</span>}
            </div>
          </div>
        </div>

        <div className="flex shrink-0 items-start justify-between gap-4 lg:flex-col lg:items-end">
          <AmountText amount={txn.amount} txnType={txn.txn_type} />
          <div className="flex justify-end gap-2">
            <Tooltip>
              <TooltipTrigger asChild>
                <Button size="icon-sm" variant="outline" aria-label="Edit category" onClick={onEditCategory}>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button size="icon-sm" variant="outline" aria-label="Edit tags" onClick={onEditTags}>
...
function TransferChainCard({
  ...
  chain: TransferChainGroup<StatementTxn>
  expanded: boolean
  onToggleExpanded: () => void
  accountsById: Record<string, AccountInfo>
  categoriesById: Record<number, CategoryWithHierarchy>
  selectedIds: Set<string>
  onToggleSelectMany: (transactionIds: string[]) => void
  onEditCategory: (txn: StatementTxn) => void
  onEditTags: (txn: StatementTxn) => void
}) {
  const chainTransactionIds = chain.members.map((member) => member.txn.id)
  const allSelected = chainTransactionIds.length > 0 && chainTransactionIds.every((id) => selectedIds.has(id))

  function renderNode(node: TransferChainGroup<StatementTxn>['roots'][number], depth = 0) {
    const category = node.txn.category ?? (node.txn.category_id != null ? categoriesById[node.txn.category_id] : undefined)

    return (
      <div key={`${node.txn.id}:${depth}`} className="space-y-3">
        <TransactionListRow
          txn={node.txn}
          account={accountsById[node.txn.account_id]}
          category={category}
          role={node.role}
          depth={depth}
          showCheckbox={false}
          onEditCategory={() => onEditCategory(node.txn)}
          onEditTags={() => onEditTags(node.txn)}
        />

        {node.children.length > 0 ? (
          <div className="space-y-3 pl-6">
            <div className="flex flex-wrap gap-2">
              {node.children.map(({ link, node: child }) => (
                <Badge key={link.id ?? `${link.fromTransactionId}:${link.toTransactionId}:${link.linkType}`} variant="outline" className="rounded-full px-3 py-1 text-[11px]">
                  {getTransferLinkLabel(link.linkType)} · {formatCurrency(link.allocatedAmount ?? Math.abs(child.txn.amount))}
                </Badge>
              ))}
            </div>
            <div className="space-y-3">
              {node.children.map(({ node: child }) => renderNode(child, depth + 1))}
            </div>
          </div>
        ) : null}
      </div>
  ...
  return (
    <div className="rounded-[1.5rem] border border-primary/20 bg-[linear-gradient(180deg,rgba(18,31,48,0.82),rgba(12,20,34,0.94))] shadow-[0_24px_80px_rgba(3,7,18,0.26)]">
      <div className="flex flex-col gap-3 p-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex min-w-0 items-start gap-3">
          <Checkbox
            checked={allSelected}
            onCheckedChange={() => onToggleSelectMany(chainTransactionIds)}
            aria-label={`Select chain ${chain.id}`}
            className="mt-1"
          />
          ...
          >
            <span className="mt-0.5 inline-flex h-8 w-8 items-center justify-center rounded-xl border border-primary/25 bg-primary/10 text-primary">
            ...
            <div className="min-w-0 space-y-2">
              <div className="flex flex-wrap items-center gap-2">
                <Badge className="rounded-full px-3 py-1 font-mono text-[11px] uppercase tracking-[0.18em]">
                  Chain · {chain.hopCount} Txn
                </Badge>
                <span className="font-mono text-[11px] uppercase tracking-[0.18em] text-muted-foreground">
                  {chain.date ? formatDateShort(chain.date) : 'Undated'}
                </span>
              </div>

              <div className="flex flex-wrap items-center gap-2 text-sm text-foreground">
                {chain.accountIds.slice(0, 5).map((accountId, index) => (
                  <div key={accountId} className="flex items-center gap-2">
                    {index > 0 ? <span className="text-muted-foreground">→</span> : null}
                    <AccountPill account={accountsById[accountId]} />
                  </div>
                ))}
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                {chain.hasMultipleDestinations ? (
                  <span className="text-xs text-muted-foreground">
                    Split across {chain.terminalCount} destinations
                  </span>
                ) : null}
              </div>
            </div>
        ...
        <div className="flex items-center gap-3 lg:flex-col lg:items-end">
          <div className="text-right">
            <div className="font-mono text-lg font-semibold text-foreground">{formatCurrency(chain.totalMoved)}</div>
            <div className="text-xs text-muted-foreground">Origin amount</div>
          </div>
        </div>
      </div>

      {expanded ? (
        <div className="border-t border-primary/15 px-4 pb-4 pt-4">
          <div className="space-y-4">
            {chain.roots.map((root) => renderNode(root))}
          </div>

          <div className="mt-5 grid gap-3 rounded-[1.15rem] border border-border/70 bg-background/35 p-4 md:grid-cols-5">
            <div>
              <div className="font-mono text-[11px] uppercase tracking-[0.18em] text-muted-foreground">Originated From</div>
              <div className="mt-1 text-sm font-medium text-foreground">{getAccountName(chain.origin ? accountsById[chain.origin.account_id] : undefined) ?? 'Unknown'}</div>
            </div>
            <div>
              <div className="font-mono text-[11px] uppercase tracking-[0.18em] text-muted-foreground">Hops</div>
              <div className="mt-1 text-sm font-medium text-foreground">{chain.hopCount}</div>
            </div>
            <div>
              <div className="font-mono text-[11px] uppercase tracking-[0.18em] text-muted-foreground">Destinations</div>
              <div className="mt-1 text-sm font-medium text-foreground">{chain.terminalCount}</div>
            </div>
            <div>
              <div className="font-mono text-[11px] uppercase tracking-[0.18em] text-muted-foreground">Total Moved</div>
              <div className="mt-1 text-sm font-medium text-foreground">{formatCurrency(chain.totalMoved)}</div>
            </div>
            <div>
              <div className="font-mono text-[11px] uppercase tracking-[0.18em] text-muted-foreground">Split</div>
              <div className="mt-1 text-sm font-medium text-foreground">
                {chain.terminalAmounts.length > 0
                  ? chain.terminalAmounts.map((amount) => formatCurrency(amount)).join(' + ')
                  : '—'}
              </div>
            </div>
          </div>
...
export default function TransactionsExplorer() {
  ...
  const roleByTransactionId = useMemo(() => {
    const map = new Map<string, 'origin' | 'passthrough' | 'terminal'>()
    for (const chain of chainGroups) {
      for (const member of chain.members) {
        map.set(member.txn.id, member.role)
      }
    }
    return map
  }, [chainGroups])
  ...
  const displayRows = useMemo(() => {
    if (viewMode !== 'all') {
      return viewFilteredTransactions.map((txn) => ({ type: 'txn', txn, date: txn.txn_date } as DisplayRow))
    }

    const visibleSet = new Set(baseFilteredTransactions.map((txn) => txn.id))
    const visibleChains = chainGroups.filter((chain) => chain.members.some((member) => visibleSet.has(member.txn.id)))
    const chainMemberIds = new Set(visibleChains.flatMap((chain) => chain.members.map((member) => member.txn.id)))
    const standaloneTransactions = baseFilteredTransactions.filter((txn) => !chainMemberIds.has(txn.id))

    const rows: DisplayRow[] = [
      ...visibleChains.map((chain) => ({ type: 'chain' as const, chain, date: chain.date ?? '' })),
      ...standaloneTransactions.map((txn) => ({ type: 'txn' as const, txn, date: txn.txn_date })),
    ]

    return rows.sort((left, right) => right.date.localeCompare(left.date))
  }, [baseFilteredTransactions, chainGroups, viewFilteredTransactions, viewMode])

  const visibleTransactionIds = useMemo(() => (
    displayRows.flatMap((row) => row.type === 'chain'
      ? row.chain.members.map((member) => member.txn.id)
      : [row.txn.id])
  ), [displayRows])

  const selectedVisibleCount = useMemo(() => visibleTransactionIds.filter((id) => selectedIds.has(id)).length, [selectedIds, visibleTransactionIds])
    ...
    <TooltipProvider>
      <ExecutivePage>
        <ExecutivePageHeader
          ...
          title="Transactions"
          description="Spending, cash flow, and full-chain audit views now share one chain-aware transaction surface."
          badges={(
            <>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <MetricCard label="Net" value={formatCurrency(metrics.net)} hint={`${displayRows.length} visible rows`} />
        </div>
        ...
        <Card className="overflow-hidden">
          <CardHeader className="border-b border-border/70">
            ...
            <CardDescription>
              Spending and cash flow flatten linked transactions. All keeps chains collapsed by default for audit and reconciliation.
            </CardDescription>
          </CardHeader>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Apply tag filters before switching views, then bulk-tag the visible rows or chain members.
            </CardDescription>
          </CardHeader>
        ...
        <Card>
          <CardHeader className="border-b border-border/70">
            <CardTitle className="flex items-center gap-2 text-base">
              ...
              {displayRows.length} rows in {getViewLedgerLabel(viewMode)}
            </CardTitle>
            <CardDescription>
              {viewMode === 'all'
                ? 'Linked transfers stay grouped as chain blocks; standalone transactions remain flat.'
                : 'Transfer chains are flattened here so only reportable terminal transactions remain visible.'}
            </CardDescription>
          </CardHeader>
            ...
            {displayRows.length === 0 ? (
              ...
              <div className="space-y-3">
                {displayRows.map((row) => {
                  if (row.type === 'chain') {
                    return (
                      <TransferChainCard
                        key={row.chain.id}
                        chain={row.chain}
                        expanded={expandedChainIds.has(row.chain.id)}
                        onToggleExpanded={() => {
                          setExpandedChainIds((current) => {
                            const next = new Set(current)
                            if (next.has(row.chain.id)) next.delete(row.chain.id)
                            else next.add(row.chain.id)
                            return next
                          })
                        }}
                  ...
                  const category = row.txn.category ?? (row.txn.category_id != null ? categoryMap[row.txn.category_id] : undefined)
                  const chain = chainByTransactionId.get(row.txn.id)
                  const role = roleByTransactionId.get(row.txn.id)
                  const transferHint = row.txn.internalTransferLink
                    ? `Transfer ${row.txn.internalTransferLink.directionLabel} ${row.txn.internalTransferLink.counterpartAccountName ?? 'linked account'}`
                    : chain && viewMode !== 'all'
                      ? `${role === 'terminal' ? 'Chain terminal' : 'Linked in chain'} · ${chain.summaryLabel}`
                      : null
                  ...
                  return (
                    <TransactionListRow
                      key={row.txn.id}
                      txn={row.txn}
                      account={accountMap[row.txn.account_id]}
                      category={category}
                      selected={selectedIds.has(row.txn.id)}
                      onToggleSelected={() => toggleSelected(row.txn.id)}
                      onEditCategory={() => openEditor(row.txn, 'category')}
                      onEditTags={() => openEditor(row.txn, 'tags')}
        ...
        {viewMode === 'all' ? (
          <Card>
            <CardHeader>
              ...
              <CardDescription>Chain rows stay collapsed by default. Expand them to inspect each origin, pass-through, and terminal node.</CardDescription>
            </CardHeader>
